...
public class BaseSpecification<T> implements Specification<T> {
    ...
    public Predicate evaluateDefaultGreaterThanOrEqual(Path<?> p, CriteriaBuilder criteriaBuilder) {
        return criteriaBuilder.greaterThanOrEqualTo(p.get(key), (Comparable) searchCriteria.getValue());
    }

    public Predicate evaluateDefaultLessThanOrEqual(Path<?> p, CriteriaBuilder criteriaBuilder) {
        return criteriaBuilder.lessThanOrEqualTo(p.get(key), (Comparable) searchCriteria.getValue());
    }

    public Predicate evaluateStringEqual(Path<?> p, CriteriaBuilder criteriaBuilder) {
        String value = (String) searchCriteria.getValue();
        return SpecificationHelper.defaultLikeQuery(criteriaBuilder, p.get(key), value);
    }
    ...
    public Predicate evaluateStringEqualCaseInsensitive(Path<?> p, CriteriaBuilder criteriaBuilder) {
        String value = (String) searchCriteria.getValue();
        return criteriaBuilder.equal(
                criteriaBuilder.lower(p.get(key)),
                value.toLowerCase()
        );
    }
    ...
    public Predicate toPredicate(@NonNull Root<T> root, @NonNull CriteriaQuery<?> query, @NonNull CriteriaBuilder criteriaBuilder) {
        ...
        if (searchCriteria.getOperation().equalsIgnoreCase(">=")) {
            return evaluateDefaultGreaterThanOrEqual(p, criteriaBuilder);
        } else if (searchCriteria.getOperation().equalsIgnoreCase("<=")) {
            return evaluateDefaultLessThanOrEqual(p, criteriaBuilder);
        } else if (searchCriteria.getOperation().equalsIgnoreCase(">")) {
            return evaluateDefaultGraterThan(p, criteriaBuilder);
        } else if (searchCriteria.getOperation().equalsIgnoreCase("<")) {
            return evaluateDefaultLessThan(p, criteriaBuilder);
        } else if (searchCriteria.getOperation().equalsIgnoreCase(":")) {
        ...
        } else if (searchCriteria.getOperation().equalsIgnoreCase("=i")) {
            return evaluateStringEqualCaseInsensitive(p, criteriaBuilder);
        }
